...
% R6.5.0(13)
    ...
    or_happens(any_alarm, T) :- %incremental_start_time(INCREMENT_T), T .>=. INCREMENT_T,
...
% R6.4.0(2)
    or_happens(basal_over_infusion_alarm, T2) :- %incremental_start_time(INCREMENT_T), T2 .>=. INCREMENT_T,
    ...
    or_happens(basal_over_infusion_alarm, T) :- %incremental_start_time(INCREMENT_T), T .>=. INCREMENT_T,
        happens(pump_has_gone_into_free_flow_valid, T), holdsAt(basal_delivery_enabled, T).
    or_happens(alarm_to_kvo, T) :- %incremental_start_time(INCREMENT_T), T .>=. INCREMENT_T,
        happens(basal_over_infusion_alarm, T).
% R6.4.0(3)
    or_happens(basal_under_infusion_warning, T2) :- %incremental_start_time(INCREMENT_T), T2 .>=. INCREMENT_T,
        ...
        not_happensInInc(basal_rate_within_tolerance_valid, T1, T2),
        not_happensInInc(basal_delivery_stopped, T1, T2).
    or_happens(basal_under_infusion_warning, T) :- %incremental_start_time(INCREMENT_T), T .>=. INCREMENT_T,
        happens(pump_flow_has_stopped_valid, T), holdsAt(basal_delivery_enabled, T).

% R6.4.0(4)
    or_happens(bolus_over_infusion_alarm, T2) :- %incremental_start_time(INCREMENT_T), T2 .>=. INCREMENT_T,
    ...
    or_happens(bolus_over_infusion_alarm, T) :- %incremental_start_time(INCREMENT_T), T .>=. INCREMENT_T,
        happens(pump_has_gone_into_free_flow_valid, T), holdsAt(patient_bolus_delivery_enabled, T).
    or_happens(alarm_to_kvo, T) :- %incremental_start_time(INCREMENT_T), T .>=. INCREMENT_T,
        happens(bolus_over_infusion_alarm, T).
% R6.4.0(5)
    or_happens(bolus_under_infusion_warning, T2) :- %incremental_start_time(INCREMENT_T), T2 .>=. INCREMENT_T,
        ...
        not_happensInInc(patient_bolus_rate_within_tolerance_valid, T1, T2),
        not_happensInInc(patient_bolus_delivery_stopped, T1, T2).
    or_happens(bolus_under_infusion_warning, T) :- %incremental_start_time(INCREMENT_T), T .>=. INCREMENT_T,
        happens(pump_flow_has_stopped_valid, T), holdsAt(patient_bolus_delivery_enabled, T).

% R6.4.0(6)
    or_happens(clinician_bolus_over_infusion_alarm, T2) :- %incremental_start_time(INCREMENT_T), T2 .>=. INCREMENT_T,
    ...
    or_happens(clinician_bolus_over_infusion_alarm, T) :- %incremental_start_time(INCREMENT_T), T .>=. INCREMENT_T,
...
% R6.4.0(7)
    or_happens(clinician_bolus_under_infusion_warning, T2) :- %incremental_start_time(INCREMENT_T), T2 .>=. INCREMENT_T,
        T2 .=. T1 + 5,                                                          % TODO fix "+ 1"
        happens(clinician_bolus_rate_under_tolerance_valid, T1),
        holdsAt(clinician_bolus_delivery_enabled, T1),
        not_happensInInc(clinician_bolus_rate_within_tolerance_valid, T1, T2),
        not_happensInInc(clinician_bolus_delivery_stopped, T1, T2).
    or_happens(clinician_bolus_under_infusion_warning, T) :- %incremental_start_time(INCREMENT_T), T .>=. INCREMENT_T,
        happens(pump_flow_has_stopped_valid, T), holdsAt(clinician_bolus_delivery_enabled, T).

% R6.4.0(8)
    or_happens(pump_overheated_alarm, T) :- %incremental_start_time(INCREMENT_T), T .>=. INCREMENT_T,
        happens(pump_temperature_over_55C_valid, T).
    or_happens(alarm_to_off, T) :- %incremental_start_time(INCREMENT_T), T .>=. INCREMENT_T,
        happens(pump_overheated_alarm, T).

% R6.1.0(4), R7.2.0(3)
    or_happens(airinline_alarm, T) :- %incremental_start_time(INCREMENT_T), T .>=. INCREMENT_T,
        happens(airinline_embolism_detected_valid, T).
    or_happens(alarm_to_off, T) :- %incremental_start_time(INCREMENT_T), T .>=. INCREMENT_T,
        happens(airinline_alarm, T).
% R6.1.0(3), R7.2.0(4), R7.2.0(6)
    or_happens(upstream_occlusion_alarm, T) :- %incremental_start_time(INCREMENT_T), T .>=. INCREMENT_T,
        happens(upstream_occlusion_detected_valid, T).
    or_happens(alarm_to_off, T) :- %incremental_start_time(INCREMENT_T), T .>=. INCREMENT_T,
        happens(upstream_occlusion_alarm, T).
% R6.1.0(2), R7.2.0(5), R7.2.0(6)
    or_happens(downstream_occlusion_alarm, T) :- %incremental_start_time(INCREMENT_T), T .>=. INCREMENT_T,
        happens(downstream_occlusion_detected_valid, T).
    or_happens(alarm_to_off, T) :- %incremental_start_time(INCREMENT_T), T .>=. INCREMENT_T,
        happens(downstream_occlusion_alarm, T).

% R7.2.0(8)
    or_happens(open_door_alarm, T) :- %incremental_start_time(INCREMENT_T), T .>=. INCREMENT_T,
