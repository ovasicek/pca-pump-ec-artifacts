...
shortcut_total_drug_in_max_dose_window_reaches_max_dose_during_clinician_bolus__0_loop(T1, BolusDurationMinutes, T2) :-
    ...
    holdsAt(total_drug_delivered(TotalBolus), T2).                                                                      %! <<< diff
shortcut_total_drug_in_max_dose_window_reaches_max_dose_during_clinician_bolus__0_loop(T1, BolusDurationMinutes, T2) :-
    % split based on diference between T1 and T2 -- crucial to avoid non termination
    initiallyP(vtbi_hard_limit_over_time(VtbiLimit, VtbiLimitTimePeriod)),
    T2 .>. T1 + VtbiLimitTimePeriod,
    ...
    TotalDuringVtbiPeriod .=. VtbiLimit,
    holdsAt(total_drug_delivered(TotalDrugAtStartPeriod), TstartVtbiPeriodCropped),                                     %! <<< diff
    holdsAt(total_drug_delivered(TotalBolus), T2).                                                                      %! <<< diff
...
shortcut_total_drug_in_max_dose_window_reaches_max_dose_during_clinician_bolus__1_dedicated_fluent(T1, BolusDurationMinutes, T2) :-
    ...
    holdsAt(clinician_bolus_total_drug_delivered(TotalBolus), T2). % dedicated fluent                                   %! <<< diff
shortcut_total_drug_in_max_dose_window_reaches_max_dose_during_clinician_bolus__1_dedicated_fluent(T1, BolusDurationMinutes, T2) :-
    % split based on diference between T1 and T2 -- crucial to avoid non termination
    initiallyP(vtbi_hard_limit_over_time(VtbiLimit, VtbiLimitTimePeriod)),
    T2 .>. T1 + VtbiLimitTimePeriod,
    % this is the trigger itself
    % needs a different implementation of "shortcut_total_drug_in_vtbi_window"
        % figure out clinician_bolus_total_drug_delivered at the start of the max dose window
        TstartVtbiPeriod .=. T2 - VtbiLimitTimePeriod,
        max(TstartVtbiPeriod, 0, TstartVtbiPeriodCropped),
        TotalDuringVtbiPeriod .=. Total - TotalDrugAtStartPeriod,
    TotalDuringVtbiPeriod .=. VtbiLimit,
    % approach 1 -- using dedicated fluents
    holdsAt(clinician_bolus_total_drug_delivered(TotalDrugAtStartPeriod), TstartVtbiPeriodCropped), % dedicated fluent  %! <<< diff
    holdsAt(clinician_bolus_total_drug_delivered(TotalBolus), T2). % dedicated fluent                                   %! <<< diff
...
shortcut_total_drug_in_max_dose_window_reaches_max_dose_during_clinician_bolus__2_plan_into_future(T1, BolusDurationMinutes, T2) :-
    ...
    trajectory(clinician_bolus_delivery_enabled(BolusDurationMinutes), T1, total_drug_delivered(TotalBolus), T2), %! <<< diff  %//NO_PREPROCESS
    % this is the trigger itself
    shortcut_total_drug_in_vtbi_window(T2, TotalBolus, VtbiLimitTimePeriod, TotalDuringVtbiPeriod),        %! <<< diff
    TotalDuringVtbiPeriod .=. VtbiLimit,
    holdsAt(total_drug_delivered(TotalBolus), T2).                              %! <<< diff
shortcut_total_drug_in_max_dose_window_reaches_max_dose_during_clinician_bolus__2_plan_into_future(T1, BolusDurationMinutes, T2) :-
    ...
    % needs a different implementation of "shortcut_total_drug_in_vtbi_window"
        ...
        max(TstartVtbiPeriod, 0, TstartVtbiPeriodCropped),
        % approach 2 -- using a trajectory definition to look into the future
        trajectory(clinician_bolus_delivery_enabled(BolusDurationMinutes), T1, total_drug_delivered(TotalDrugAtStartPeriod), TstartVtbiPeriodCropped), %! <<< diff  %//NO_PREPROCESS 
        TotalDuringVtbiPeriod .=. Total - TotalDrugAtStartPeriod,
    TotalDuringVtbiPeriod .=. VtbiLimit,
    holdsAt(total_drug_delivered(TotalDrugAtStartPeriod), TstartVtbiPeriodCropped), %! <<< diff
    holdsAt(total_drug_delivered(TotalBolus), T2).                              %! <<< diff
...
shortcut_total_drug_in_max_dose_window_reaches_max_dose_during_clinician_bolus__5_combine_1_2(T1, BolusDurationMinutes, T2) :-
    ...
    T2 .=<. T1 + VtbiLimitTimePeriod,
    % approach 2 -- using a trajectory definition to look into the future
    trajectory(clinician_bolus_delivery_enabled(BolusDurationMinutes), T1, clinician_bolus_total_drug_delivered(TotalBolus), T2), %! <<< diff  %//NO_PREPROCESS
    % this is the trigger itself
    shortcut_total_drug_in_vtbi_window(T2, TotalBolus, VtbiLimitTimePeriod, TotalDuringVtbiPeriod),        %! <<< diff
    TotalDuringVtbiPeriod .=. VtbiLimit,
    % approach 1 -- using a dedicated fluent
...
shortcut_total_drug_in_max_dose_window_reaches_max_dose_during_clinician_bolus__5_combine_1_2(T1, BolusDurationMinutes, T2) :-
    ...
    % needs a different implementation of "shortcut_total_drug_in_vtbi_window"
        ...
        max(TstartVtbiPeriod, 0, TstartVtbiPeriodCropped),
        % approach 2 -- using a trajectory definition to look into the future
        trajectory(clinician_bolus_delivery_enabled(BolusDurationMinutes), T1, clinician_bolus_total_drug_delivered(TotalDrugAtStartPeriod), TstartVtbiPeriodCropped), %! <<< diff  %//NO_PREPROCESS 
        TotalDuringVtbiPeriod .=. Total - TotalDrugAtStartPeriod,
    TotalDuringVtbiPeriod .=. VtbiLimit,
    % approach 1 -- using dedicated fluents
    holdsAt(clinician_bolus_total_drug_delivered(TotalDrugAtStartPeriod), TstartVtbiPeriodCropped), % dedicated fluent %! <<< diff
...
shortcut_total_drug_in_max_dose_window_reaches_max_dose_during_clinician_bolus__7_combine_1_4(T1, BolusDurationMinutes, T2) :-
    ...
    shortcut_total_drug_in_vtbi_window(T2, TotalBolus, VtbiLimitTimePeriod, TotalDuringVtbiPeriod),        %! <<< diff
    TotalDuringVtbiPeriod .=. VtbiLimit,
    holdsAt(clinician_bolus_total_drug_delivered(TotalBolus), T2, clinician_bolus_delivery_enabled(_)).                              %! <<< diff
shortcut_total_drug_in_max_dose_window_reaches_max_dose_during_clinician_bolus__7_combine_1_4(T1, BolusDurationMinutes, T2) :-
    % split based on diference between T1 and T2 -- crucial to avoid non termination
    initiallyP(vtbi_hard_limit_over_time(VtbiLimit, VtbiLimitTimePeriod)),
    T2 .>. T1 + VtbiLimitTimePeriod,
    ...
    % needs a different implementation of "shortcut_total_drug_in_vtbi_window"
        ...
        TotalDuringVtbiPeriod .=. Total - TotalDrugAtStartPeriod,
    TotalDuringVtbiPeriod .=. VtbiLimit,
    holdsAt(clinician_bolus_total_drug_delivered(TotalDrugAtStartPeriod), TstartVtbiPeriodCropped, clinician_bolus_delivery_enabled(_)), %! <<< diff
    holdsAt(clinician_bolus_total_drug_delivered(TotalBolus), T2, clinician_bolus_delivery_enabled(_)).                              %! <<< diff
