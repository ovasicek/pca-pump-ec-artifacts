...
% R6.8.0(10), R7.2.0(7)
    ...
    or_happens(empty_reservoir_alarm_EFFECT, T) :-
    ...
    or_happens(empty_reservoir_alarm, T) :-
    ...
    or_happens(kvo_halted_empty_reservoir, T) :-
        T2 .<. T,
...
% R6.8.0(9)
    ...
    or_happens(low_reservoir_warning, T) :-
        ...
        DrugLeft .=<. LowThreshold, DrugLeft .>. EmptyThreshold, holdsAt(drug_reservoir_contents(DrugLeft), T).


    %! avoiding infinite loops by using the self-ending trajectory approach (similar to triggering bolus completed)
    % low reservoir during basal delivery
    or_happens(basal_halted_low_reservoir, T) :-
    ...
    or_happens(low_reservoir_warning_EFFECT, T) :-
    ...
    or_happens(patient_bolus_halted_low_reservoir, T) :-
        initiallyP(low_reservoir_treshold(Threshold)),
        initiallyP(initial_drug_reservoir_contents(StartingDrug)),
        Threshold .=. StartingDrug - UsedDrug,
    ...
    or_happens(clinician_bolus_halted_low_reservoir, T) :-
        initiallyP(low_reservoir_treshold(Threshold)),
        initiallyP(initial_drug_reservoir_contents(StartingDrug)),
        Threshold .=. StartingDrug - UsedDrug,
    ...
    or_happens(kvo_low_reservoir, T) :-
        initiallyP(low_reservoir_treshold(Threshold)),
        initiallyP(initial_drug_reservoir_contents(StartingDrug)),
        Threshold .=. StartingDrug - UsedDrug,
        holdsAt(total_drug_delivered(UsedDrug), T, kvo_delivery_enabled).
