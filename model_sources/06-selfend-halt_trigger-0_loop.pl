% basic non-terminating approach -- loops because holdsAt(total_bolus_drug_delivered(TotalT2),T2) can be determined by other trajectories which can be started by this trigger
or_happens(clinician_bolus_halted_max_dose, T2) :-
    ...
    shortcut_total_drug_in_max_dose_window_reaches_max_dose_during_clinician_bolus__0_loop(T1, DurationMinutes, T2).