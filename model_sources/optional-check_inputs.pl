...
%:- initiallyP(patient_bolus_flow_rate_MlperHour(X)), initiallyP(vtbi_hard_max(Max)), X .>. Max.  % R5.2.0(4)
...
:- initiallyP(vtbi_hard_limit_over_time(MaxDose, MaxDosePeriod)), MaxDose .<. BasalRate * MaxDosePeriod, initiallyP(basal_flow_rate(BasalRate)).

% the max dose time period must be bigger than the patient bolus duration
:- initiallyP(vtbi_hard_limit_over_time(_, MaxDosePeriod)), patient_bolus_duration(BolusDuration), MaxDosePeriod .<. BolusDuration.
