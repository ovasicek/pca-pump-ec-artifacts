...
or_happens(patient_bolus_requested,                 239/2). % Pre 1 (119.5)
...
or_happens(stop_button_pressed,                     120).   % Pre 1
    ...
    ?- not_holdsAt(patient_bolus_delivery_enabled,  121).   % Pre 1
...
?-  holdsAt(basal_delivery_enabled,                 70),
    ...
    not_holdsAt(patient_bolus_delivery_enabled,     121),
