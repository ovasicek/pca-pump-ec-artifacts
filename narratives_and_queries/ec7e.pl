...
or_happens(clinician_bolus_rate_over_tolerance,         120).   % Step 1e
    ...
    ?- happens(clinician_bolus_halted,                  121).   % Step 3            %! failure 1
    ?- happens(kvo_delivery_started,                    121).   % Step 3            %! failure 1
    ?- holdsAfter(kvo_delivery_enabled,                 121).   % Step 3            %! failure 1

    ?- holdsAfter(alarm_active,                         121).   % Post 1            %! failure 1
    
    ?- not_holdsAfter(pump_running,                     121).   % Post 2            %! failure 2
 %?%?- holdsAfter(kvo_delivery_enabled,                 121).   % Post 2            % TODO fix 2
...
?-  holdsIn(basal_delivery_enabled,                 60, 118),
    ...
    happens(clinician_bolus_halted,                     121),
    happens(kvo_delivery_started,                       121),
    holdsAfter(kvo_delivery_enabled,                    121),
    holdsAfter(alarm_active,                            121),
    
    not_holdsAfter(pump_running,                        121).
 %?%holdsAfter(kvo_delivery_enabled,                    121).
